        ...
        if not errors:
            with st.spinner("Processing all receipts..."):
                try:
                    ...
                    # ── FIX: scope the source to THIS receipt before any account split ──
                    # In the source file, the parent and all its children share the SAME
                    # ReceiptNumber (the parent's real HR number); the _1/_2 suffixes are
                    # synthetic, output-only. Without this filter, an account-only split
                    # (Item Account Number == acct) pulls in EVERY other receipt in the file
                    # that happens to share the same customer account — collapsing multiple
                    # same-customer parent/child receipts into one and blowing up reconciliation.
                    src_full['ReceiptNumber'] = src_full['ReceiptNumber'].str.strip()
                    base_rn = parent['receipt_number'].strip().split('_')[0]
                    src_full = src_full[src_full['ReceiptNumber'] == base_rn].copy()
                    if src_full.empty:
                        st.markdown(
                            f'<div class="status-err">✗ No rows for receipt {base_rn} in source file. '
                            f'Check that the Parent receipt number matches the ReceiptNumber in the file.</div>',
                            unsafe_allow_html=True)
                        st.stop()
